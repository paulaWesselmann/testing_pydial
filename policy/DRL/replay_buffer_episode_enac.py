...
class ReplayBufferEpisode(replay_abc.ReplayABC):
    ...
    def record(self, state, state_ori, action, reward, value, distribution, terminal=False):
        ...
        if self.s_prev == None and self.s_ori_prev == None and self.a_prev == None and self.r_prev == None and self.distribution_prev == None:
            ...
            if terminal == True:
                ...
                self.episode.append(\
                    [self.s_prev, self.s_ori_prev, self.a_prev, self.r_prev, self.v_prev, state, state_ori, terminal, self.distribution_prev]\
                )
                self.s_prev, self.s_ori_prev, self.a_prev, self.r_prev, self.v_prev, self.distribution_prev = state, state_ori, action, reward, value, distribution
    def size(self):
        return self.count
    ...
    def sample_batch(self):
        ...
        mu_batch        = []
        for epi in batch:
            tmp_s, tmp_s_ori, tmp_a, tmp_r, tmp_v, tmp_s2, tmp_s2_ori, tmp_t, tmp_mu = \
                [], [], [], [], [], [], [], [], []
            for exp in epi:
                tmp_s.append(exp[0])
