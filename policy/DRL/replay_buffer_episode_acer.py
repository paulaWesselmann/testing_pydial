...
class ReplayBufferEpisode():
    ...
    def record(self, state, state_ori, action, reward, value, distribution, terminal=False, mask=None):
        """
        ...
        if self.s_prev is None and self.s_ori_prev is None and self.a_prev is None and self.r_prev is None and \
                self.distribution is None and self.mask is None:
            self.s_prev, self.s_ori_prev, self.a_prev, self.r_prev, self.v_prev, self.distribution, self.mask = \
            ...
            if terminal == True:
                ...
                self.episode.append( \
                    [self.s_prev, self.s_ori_prev, self.a_prev, self.r_prev, state, state_ori, self.v_prev, terminal,
    ...
    def sample_batch(self):
        ...
        return_mask = False
        for epi in batch:
            tmp_s, tmp_s_ori, tmp_a, tmp_r, tmp_s2, tmp_s2_ori, tmp_v, tmp_t, tmp_d, tmp_m = \
                [], [], [], [], [], [], [], [], [], []
            for exp in epi:
                tmp_s.append(exp[0])
        ...
        ret = [s_batch, s_ori_batch, a_batch, r_batch, s2_batch, s2_ori_batch, t_batch, None, v_batch, d_batch]
        if return_mask:
            ret.append(m_batch)
        return ret
